...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="docdw/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/dallocdalloc/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/dallocdalloc/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://dallocdalloc:${GITHUB_ACCESS_TOKEN}@github.com/dallocdalloc/guestbook-config.git"
                    '''
                }
            }
        }
